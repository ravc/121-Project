...
public:
    add_cust_window(Point xy, int w, int h, const string& title,vector<customers>& c);
private:
	//buttons etc
...
add_cust_window::add_cust_window(Point xy,int w,int h,const string& title,vector<customers>& c):
	Window(xy,w,h,title),
	listOfCustomers(c),
...
void add_cust_window::proceeder(){
	...
	//c.set_photo_filename(photoin);
	listOfCustomers.push_back(c);//add customer to vector
	hide();
}
struct remove_cust_window : Graph_lib::Window{
public:
    remove_cust_window(Point xy, int w, int h, const string& title,vector<customers> c);
private:
	vector<customers> listOfCustomers;
	//buttons etc
	In_box name;
...
remove_cust_window::remove_cust_window(Point xy,int w,int h, const string& title,vector<customers> c): 
		Window(xy,w,h,title),
		listOfCustomers(c),
		name(Point(170,100),100,20,"customer name: "),
...
public:
	...
	add_tag_window(Point xy,int w,int h,const string& title,string n,string a,string p,Geo_Loc g,vector<string> t,vector<Place_Info>& pl);
private:
	//vstring tagget;
...
add_tag_window::add_tag_window(Point xy,int w,int h,const string& title,string n,string a,string p,Geo_Loc g,vector<string> t,vector<Place_Info>& pl): Window(xy,w,h,title),tag(Point(110,110),100,20,"tag"),proceed(Point(250,330),140,20,"add another tag",cb_proceed),
quit_button(Point(290,350),100,40,"no more tags\n(create place)",cb_quit),namein(n),addyin(a),photoin(p),gloc(g),tags_in(t),listOfPlaces(pl){
	attach(tag);
	attach(quit_button);
	attach(proceed);
}
...
int add_tag_window::proceeder(){
	...
	while (true){//open a new window, get a new tag
    	add_tag_window win2(Point(100,100),400,400,"tag",namein,addyin,photoin,gloc,tags_in,listOfPlaces);
    	return gui_main();
	}
}
struct add_place_window : Graph_lib::Window{
public:
    add_place_window(Point xy, int w, int h, const string& title,vector<Place_Info>& p);
private:
...
add_place_window::add_place_window(Point xy,int w,int h,const string& title,vector<Place_Info>& p):
	Window(xy,w,h,title),
	listOfPlaces(p),
...
int add_place_window::proceeder(){
	...
	hide();
	while (true){//open a new window, get a new tag
    	add_tag_window win2(Point(100,100),400,400,"tag",namein,addyin,photoin,g,tagsin,listOfPlaces);
    	return gui_main();
	}
...
public:
    remove_place_window(Point xy, int w, int h, const string& title,vector<Place_Info>& p);
private:
	//buttons etc
	vector<Place_Info> listOfPlaces;
	In_box name;
...
remove_place_window::remove_place_window(Point xy,int w,int h, const string& title,vector<Place_Info>& p): 
		Window(xy,w,h,title),
		listOfPlaces(p),
...
public:
    add_driver_window(Point xy, int w, int h, const string& title,vector<drivers>& d);
private:
	//buttons etc
...
add_driver_window::add_driver_window(Point xy,int w,int h,const string& title,vector<drivers>& d):
	Window(xy,w,h,title),
	listOfDrivers(d),
...
public:
    remove_driver_window(Point xy, int w, int h, const string& title,vector<drivers>& d);
private:
	//buttons etc
	vector<drivers> listOfDrivers;
	In_box name;
...
remove_driver_window::remove_driver_window(Point xy,int w,int h, const string& title,vector<drivers>& d): 
		Window(xy,w,h,title),
		listOfDrivers(d),
		name(Point(170,100),100,20,"driver name: "),
		proceed(Point(320,340),70,20,"remove",cb_proceed),
...
void remove_driver_window::proceeder(){
	...
	hide();
>>>>>>> 4c3d55858a681b4f09b517a0a22e2ea18ae6d087
}